...
function usage {
  echo "Usage: graceful_stop.sh [--config <conf-dir>] [-d] [-e] [--restart [--reload [-x <excludefile>]]] [--thrift] [--rest] <hostname>"
  ...
  echo " x|exclude      Move regions to other region servers except exclude ones"
...
do
  case "$1" in
    ...
    --exclude | -x)  shift; excludefile="-x $1"; shift;;
...
HBASE_NOEXEC=true "$bin"/hbase --config ${HBASE_CONF_DIR} org.jruby.Main "$bin"/region_mover_patch.rb --file=$filename $debug $excludefile --maxthreads=$maxthreads unload $hostname
...
if [ "$restart" != "" ]; then
  ...
  if [ "$reload" != "" ]; then
    ...
    HBASE_NOEXEC=true "$bin"/hbase --config ${HBASE_CONF_DIR} org.jruby.Main "$bin"/region_mover_patch.rb --file=$filename $debug --maxthreads=$maxthreads load $hostname
